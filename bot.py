...
def score_local(text):
    ...
    # Phrase rules: combinations give higher priority
    for words, cat, pts in PHRASE_RULES:
        if all(w.lower() in lo for w in words):
            sc[cat] = sc.get(cat, 0) + pts
    return sc

def best_cats(sc):
    if not sc: return ['General']
    s = sorted(sc.items(), key=lambda x: x[1], reverse=True)
    top = s[0][1]
    return [c for c, v in s if v >= top * 0.6]
